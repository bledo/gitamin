...
{
	final static Logger log = LoggerFactory.getLogger(RestConnector.class);
	...
	public RestConnector(Auth auth)
	{
		this.cookies = auth.cookies;
		this.host = auth.HOST;
		this.port = auth.PORT;
		this.domain = auth.DOMAIN;
		this.project = auth.PROJECT;
	}
	...
	private Response doHttp( String type, String url, String queryString,
		byte[] data, Map<String, String> headers, Map<String, String> cookies) throws Exception
	{
		if ((queryString != null) && !queryString.isEmpty())
		{
			url += "?" + queryString;
		}
		log.debug("TestDirector request url {}", url);
